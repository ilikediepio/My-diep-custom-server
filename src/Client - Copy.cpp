...
import { CameraFlags, ClientBound, ArenaFlags, InputFlags, NameFlags, PhysicsFlags, ServerBound, Stat, StatCount, Tank } from "./Const/Enums";
import { AI, AIState, Inputs } from "./Entity/AI";
import AbstractBoss from "./Entity/Boss/AbstractBoss";
import { executeCommand } from "./Const/Commands";
import { bannedClients } from ".";
import { getDefaultAccessLevelForArena, AccessLevel, defaultAccessLevel } from "./config";
import fs from "fs";
import { logJoin, frozenIPs, loadFrozen, canonicalIP } from "../devtools/admin-cli";
...
export default class Client {
    ...
    public chatToggled: boolean = true;
...
//private canonicalIP(ip: string): string {
    ...
    private handleIncoming(header: number, data: Uint8Array): void {
        ...
        if (header === ServerBound.Init) {
            ...
            if (config.devPasswordHash && createHash("sha256").update(pw).digest("hex") === config.devPasswordHash) {
                ...
                if (this.accessLevel !== AccessLevel.FullAccess) {
                this.accessLevel = getDefaultAccessLevelForArena(this.game);
            }
            }
            ...
            const ws = this.ws;
        if (!ws) return this.terminate();

        const ip = canonicalIP(ws.getUserData().ipAddress);

        console.log("BAN CHECK:", ip, bannedClients);

// check active bans
//const bans = JSON.parse(fs.readFileSync("devtools/bans.json", "utf8"));

// load bans
let bans: any[] = [];
try {
    bans = JSON.parse(fs.readFileSync("devtools/bans.json", "utf8"));
} catch {
    bans = [];
}

// find matching ban
const ban = bans.find((b: any) => canonicalIP(b.ip) === ip);

// check expiry
if (ban && ban.expiresAt > Date.now()) {
    util.log(`Blocked banned IP: ${ip}`);
    this.terminate();
    return;
}
...
const rawIP = this.ws?.getUserData().ipAddress ?? "";
const ip = canonicalIP(rawIP);

//const frozen = ip ? loadFrozen() : null;
loadFrozen();

if (ip && frozenIPs.has(ip)) {

    this.inputs.flags = 0;
    this.inputs.movement.x = 0;
    this.inputs.movement.y = 0;
    this.inputs.movement.magnitude = 0;
    ...
    return;
                ...
                if (flags & InputFlags.levelup) {
                    if ((this.accessLevel >= config.AccessLevel.BetaAccess) || ((this.accessLevel >= config.AccessLevel.kReserved) && (camera.cameraData.values.level < config.maxPlayerLevel))) {
                        this.setHasCheated(true);
            ...
            case ServerBound.Spawn: {
                ...
                const ip = canonicalIP(this.ws!.getUserData().ipAddress);
                logJoin(ip, name);
...
case ServerBound.TCPInit:
    const rawIP: string = this.ws?.getUserData().ipAddress ?? "";
    const ip: string = canonicalIP(rawIP);

    if (ip && frozenIPs.has(ip)) {
        console.log("COMMAND BLOCKED:", ip);
        this.notify("You are frozen and cannot use commands.", 0xFF0000, 3500);
        return;
    }
    ...
    public possess(ai: AI) {
        ...
        this.camera.cameraData.tank = 10;
    ...
    public ban() {
        ...
        bannedClients.set(ws.getUserData().ipAddress, Infinity);
    ...
    public tick(tick: number) {
        ...
        const rawIP = this.ws?.getUserData().ipAddress;
        const ip = rawIP ? canonicalIP(rawIP) : null;

        //const frozen = ip ? loadFrozen() : null;
        loadFrozen();

        //console.log("TICK IP:", rawIP, "->", ip, "Frozen:", frozenIPs);

        if (ip && frozenIPs.has(ip)) {
            if (process.env.NODE_ENV === "development") {
    console.log("FREEZE ACTIVE:", ip);
            }
...
if (process.env.NODE_ENV === "development") {
        console.log("SERVER frozenIPs:", frozenIPs);
}
