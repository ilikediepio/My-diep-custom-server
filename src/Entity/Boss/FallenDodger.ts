 ...
 */
export default class FallenDodger extends AbstractBoss {

    public movementSpeed = 1.75;

    private teleportCooldown = 500;
    private readonly TELEPORT_DISTANCE = 75;
    private readonly TELEPORT_TRIGGER_DISTANCE = 300;

    public constructor(game: GameServer) {
        super(game);

        this.nameData.values.name = 'Fallen Dodger';

        for (const barrelDefinition of TankDefinitions[Tank.Sniper].barrels) {
            const def = Object.assign({}, barrelDefinition, {});
            def.bullet = Object.assign({}, def.bullet, {
                speed: 2,
                health: 14,
                damage: def.bullet.damage * 2
            });
