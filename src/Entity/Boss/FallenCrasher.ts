...
/**
 * Class which represents the boss "FallenCrasher"
 */
export default class FallenCrasher extends AbstractBoss {
    /** The speed to maintain during movement. */
    public movementSpeed = 1.5;

    public constructor(game: GameServer) {
        super(game);

        this.nameData.values.name = 'Fallen Crasher';
        this.physicsData.values.sides = 3;
        for (const barrelDefinition of TankDefinitions[Tank.FallenCrasher].barrels) {

            const def = Object.assign({}, barrelDefinition, {});
            def.bullet = Object.assign({}, def.bullet, { speed: 1.8, health: 30, damage:def.bullet.damage * 1.5 });
            this.barrels.push(new Barrel(this, def));
        }
    }

    public get sizeFactor() {
        return this.physicsData.values.size / 75;
    }
