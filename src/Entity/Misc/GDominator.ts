 ...
 * Dominator Tank
 */
export default class GDominator extends TankBody {
    /** Size of a subdominator */
    public static SIZE = 120;
    ...
    public constructor(arena: ArenaEntity, base: TeamBase, pTankId: Tank | null = null) {
        const tankId: Tank = pTankId ?? Tank.DominatorG;

        const inputs = new Inputs();
        const camera = new CameraEntity(arena.game);

        camera.setLevel(75);
        camera.sizeFactor = (GDominator.SIZE / 50);

        super(arena.game, camera, inputs);

        this.relationsData.values.team = arena;
        this.physicsData.values.size = GDominator.SIZE;
        // TODO(ABC):
        ...
        this.nameData.values.name = "Sub Dominator";
