...
export default class Terrestrial extends TankBody {
    ...
    public constructor(game: GameServer) {
        ...
        this.ai.viewRange = 25000; // Keep AI's view range same as bossses

if (Math.random() < 0.5) {
    this.setTank(Tank.Chr);
    this.nameData.values.name = "Chronosyn The Boundless";
    this.styleData.values.color = Color.Gold;
    this.damagePerTick = 150;
} else {
    this.setTank(Tank.Ragnarok);
    this.nameData.values.name = "Vorlax The Undying";
    this.styleData.values.color = Color.DarkPurple;
    this.damagePerTick = 400;
}

        const def = (this.definition = Object.assign({}, this.definition));
        def.maxHealth = 200000;
        def.speed = 0.15; // Keep speed constant, independent of level

        this.healthData.values.health = 200000;

        camera.cameraData.values.player = this;

        // Set the level and score explicitly to 120 and 700000
        camera.cameraData.values.level = 300;
        camera.cameraData.values.score = 100000000;
    ...
    public tick(tick: number) {
        ...
        this.ai.movementSpeed = 0.25 * 10;  // Use the fixed speed here
