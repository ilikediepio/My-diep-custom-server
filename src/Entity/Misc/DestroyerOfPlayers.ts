 ...
 */
export default class Triacosiahexecontagon extends TankBody {
    /** Size of a large boss. */
    public static BASE_SIZE = 200;
    ...
    public constructor(game: GameServer) {
        ...
        camera.setLevel = function(level) {
            ...
            this.sizeFactor = (Triacosiahexecontagon.BASE_SIZE / 50); // Keep size constant
        }
        camera.sizeFactor = (Triacosiahexecontagon.BASE_SIZE / 50); // Fix sizeFactor at a constant value
        ...
        this.ai.viewRange = 30000;

    this.setTank(Tank.HexadecagonBoss);
    this.nameData.values.name = "Triacosiahexecontagon the vengeful";
    this.styleData.values.color = Color.UltraDarkPurple;

        const def = (this.definition = Object.assign({}, this.definition));
        def.maxHealth = 10000;
        def.speed = 2; // Keep speed constant, independent of level

        this.damagePerTick = 100;
        this.healthData.values.health = 10000;

        camera.cameraData.values.player = this;

        // Set the level and score explicitly to 180 and 50 million
        camera.cameraData.values.level = 180;
        camera.cameraData.values.score = 50000000;
    ...
    public tick(tick: number) {
        ...
        this.ai.movementSpeed = 2 * 10;  // Use the fixed speed here
