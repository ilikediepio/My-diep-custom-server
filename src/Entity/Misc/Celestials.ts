...
if (Math.random() < 0.5) {
    this.setTank(Tank.Freyja);
    this.nameData.values.name = "Noctyra";
    this.styleData.values.color = Color.Shiny;
} else {
    this.setTank(Tank.Nyx);
    this.nameData.values.name = "Solara";
    this.styleData.values.color = Color.EnemySquare;
}

        const def = (this.definition = Object.assign({}, this.definition));
        def.maxHealth = 20000;
        def.speed = 0.5; // Keep speed constant, independent of level

        this.damagePerTick = 30;
        this.healthData.values.health = 20000;

        camera.cameraData.values.player = this;

        // Set the level and score explicitly to 120 and 700000
        camera.cameraData.values.level = 175;
    ...
    public tick(tick: number) {
        ...
        this.ai.movementSpeed = 0.4 * 10;  // Use the fixed speed here
