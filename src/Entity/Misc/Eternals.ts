...
if (Math.random() < 0.5) {
    this.setTank(Tank.Kronos);
    this.nameData.values.name = "Kronos";
    this.styleData.values.color = Color.White;
} else {
    this.setTank(Tank.Ragnarok);
    this.nameData.values.name = "Ragnarock";
    this.styleData.values.color = Color.Hexagon;
}

        const def = (this.definition = Object.assign({}, this.definition));
        def.maxHealth = 75000;
        def.speed = 0.25; // Keep speed constant, independent of level

        this.damagePerTick = 50;
        this.healthData.values.health = 75000;

        camera.cameraData.values.player = this;

        // Set the level and score explicitly to 120 and 700000
        camera.cameraData.values.level = 250;
        camera.cameraData.values.score = 5000000;
