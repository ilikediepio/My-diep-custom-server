...
import { Color, NameFlags, StyleFlags, AdminFlags, Tank } from "../../Const/Enums";
 ...
 * Dominator Tank
 */
export default class AcDominator extends TankBody {
    /** Size of an acdominator */
    ...
    public constructor(arena: ArenaEntity, base: TeamBase, pTankId: Tank | null = null) {
        const tankId: Tank = pTankId ?? Tank.ArenaCloser;

        const inputs = new Inputs();
        const camera = new CameraEntity(arena.game);

        camera.setLevel(75);
        camera.sizeFactor = (AcDominator.SIZE / 50);

        super(arena.game, camera, inputs);

        this.relationsData.values.team = arena;
        this.physicsData.values.size = AcDominator.SIZE;
        // TODO(ABC):
        ...
        this.nameData.values.name = "Arena Dominator";
        //this.nameData.values.flags |= NameFlags.hiddenName;
        this.physicsData.values.absorbtionFactor = 0;
        
        camera.cameraData.values.level = 75;
