...
import { InputFlags, PhysicsFlags, AdminFlags } from "../Const/Enums";
...
export class AI {
    ...
    public onlyTargetPlayers: boolean = false;
    ...
    public findTarget(tick: number) {
        ...
        const applyForwardCone = !!(this.owner.flags.adminFlags & AdminFlags.targetFront);
        const FIRE_CONE = Math.PI / 6; // 30 degree cone
        const ownerPos = this.owner.getWorldPosition();
        const ownerAngle = this.owner.positionData.values.angle;
        ...
        for (let i = 0; i < entities.length; ++i) {
            ...
            if (this.onlyTargetPlayers && (!(entity instanceof TankBody) || entity.relationsData.values.owner === null)) continue;
...
const toTarget = Math.atan2(
    entity.positionData.values.y - ownerPos.y,
    entity.positionData.values.x - ownerPos.x
);

    let angleDiff = ((toTarget - ownerAngle + Math.PI) % (2 * Math.PI)) - Math.PI;

    if (Math.abs(angleDiff) > FIRE_CONE / 2) continue; // skip targets outside the front cone
}
...
public tick(tick: number) {
    // If its being possessed, but its possessor is deleted... then just restart;
    ...
    if (!target) {
        ...
        // Check if the target is still in front if targetFront flag is set
        if (applyForwardCone) {
            const toTarget = Math.atan2(
                target.positionData.values.y - ownerPos.y,
                target.positionData.values.x - ownerPos.x
            );

            const FIRE_CONE = Math.PI / 6; // 30 degrees
            let angleDiff = ((toTarget - ownerAngle + Math.PI) % (2 * Math.PI)) - Math.PI;

            if (Math.abs(angleDiff) > FIRE_CONE / 2) {
                // Target is no longer in front, stop shooting
