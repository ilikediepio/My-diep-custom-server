...
export const BMultiAutoRocketDefinition: BarrelDefinition = {
    angle: 0,
    offset: 0,
    size: 50,
    width: 40 * 0.7,
...
export default class BMultiAutoRocket extends MultiAutoBase {
    ...
    public constructor(owner: BarrelBase, turretDefinition: BarrelDefinition = BMultiAutoRocketDefinition, baseSize: number = 20) {
        super(owner);
    ...
    // Add a static helper to create multiple turrets (optional but recommended)
//    public static createBMultiAutoRockets(owner: BarrelBase, count: number): BMultiAutoRocket[] {
//        const turrets: BMultiAutoRocket[] = [];
//        for (let i = 0; i < count; i++) {
//            turrets.push(new BMultiAutoRocket(owner));
//        }
//        return turrets;
//    }
    ...
    public static createBMultiAutoRockets(owner: BarrelBase, count: number): BMultiAutoRocket[] {
    const turrets: BMultiAutoRocket[] = [];
    const radius = owner.sizeFactor * 400; // Adjust how far turrets appear from tank

    for (let i = 0; i < count; i++) {
        const angle = (i / count) * 2 * Math.PI;

        const turret = new BMultiAutoRocket(owner);
        ...
        turret.positionData.values.angle = angle;
