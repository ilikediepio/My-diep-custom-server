 ...
 */
export default class MultiMultiMinion extends Drone implements BarrelBase {
    ...
    protected tickMixin(tick: number) {
        ...
        if (usingAI && this.ai.state === AIState.idle) {
            ...
            if (dist < MultiMultiMinion.FOCUS_RADIUS / 4) { // Half
                this.movementAngle = this.positionData.values.angle + Math.PI;
            } else if (dist < MultiMultiMinion.FOCUS_RADIUS) {
