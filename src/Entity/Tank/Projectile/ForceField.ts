...
import { HealthFlags, PositionFlags, PhysicsFlags, AdminFlags, Stat, StyleFlags, Color } from "../../../Const/Enums";
 ...
 */
export default class ForceFeild extends LivingEntity {
    ...
    public constructor(barrel: Barrel, tank: BarrelBase, tankDefinition: TankDefinition | null, shootAngle: number) {
        ...
        this.styleData.values.color = Color.Field;
        this.styleData.values.flags |= StyleFlags.hasNoDmgIndicator;
        this.healthData.values.flags = HealthFlags.hiddenHealthbar;
        this.flags.adminFlags |= AdminFlags.instakill;
    ...
    public tick(tick: number) {
     ...
     /*   if ((this.relationsData.values.team?.entityState || 0) & EntityStateFlags.needsDelete) this.relationsData.values.team = null
        ...
        for (const entity of this.game.entities.internalMap.values()) {
        if (!e.alive) continue;
        if (!(e instanceof Bullet)) continue;
        if (e.relationsData.values.team === this.relationsData.values.team) continue;

        const dx = e.positionData.values.x - this.positionData.values.x;
        const dy = e.positionData.values.y - this.positionData.values.y;
        const dist = Math.sqrt(dx * dx + dy * dy);

        const radiusSum = this.physicsData.values.size / 2 + e.physicsData.values.size / 2;
        if (dist < radiusSum) {
            e.destroy(); // bullet destroyed
         }
      } */
   }
}