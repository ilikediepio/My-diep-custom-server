 ...
 */
export default class BulletRed extends LivingEntity {
    ...
    protected deathAccelFactor = 0; //0.5
    ...
    public constructor(barrel: Barrel, tank: BarrelBase, tankDefinition: TankDefinition | null, shootAngle: number) {
        ...
        this.styleData.values.color = Color.Lblue;
...
public fastDestroy() {
    ...
    // Instantly trigger death fade, but faster
    const fadeInterval = 5; // how many ticks between fades (smaller = faster)

    const fadeStep = 1 / 10; // near instant death anamation
    const fadeTimer = setInterval(() => {
        this.styleData.values.opacity -= fadeStep;
        if (this.styleData.values.opacity <= 0) {
            clearInterval(fadeTimer);
            super.destroy(false); // finish deletion with no extra animation
        }
    }, fadeInterval);
}
    ...
    public tick(tick: number) {
        ...
        if (tick - this.spawnTick >= this.lifeLength) this.fastDestroy();
        // TODO(ABC):
