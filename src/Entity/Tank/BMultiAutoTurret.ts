...
export const BMultiAutoTurretDefinition: BarrelDefinition = {
    angle: 0,
    offset: 0,
    size: 45,
    width: 35 * 0.7,
...
export default class BMultiAutoTurret extends ObjectEntity {
    ...
    public constructor(owner: BarrelBase, turretDefinition: BarrelDefinition = BMultiAutoTurretDefinition, baseSize: number = 30) {
        super(owner.game);
...
//            turrets.push(new MultiAutoRocket(owner));
//        }
//        return turrets;
//    }

    public static createBMultiAutoTurrets(owner: BarrelBase, count: number): BMultiAutoTurret[] {
        const turrets: BMultiAutoTurret[] = [];
    const radius = owner.sizeFactor * 400; // Adjust how far turrets appear from tank

    for (let i = 0; i < count; i++) {
        const angle = (i / count) * 2 * Math.PI;

        const turret = new BMultiAutoTurret(owner);
