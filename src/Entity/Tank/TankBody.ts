...
import { Color, StyleFlags, StatCount, Tank, CameraFlags, Stat, InputFlags, PhysicsFlags, PositionFlags, AdminFlags } from "../../Const/Enums";
...
export default class TankBody extends LivingEntity implements BarrelBase {
    ...
    public constructor(game: GameServer, camera: CameraEntity, inputs: Inputs) {
        ...
        //this.flags.adminFlags |= AdminFlags.immuneToKillCommand; this code is glitchy and makes ac and bots immune to kill command
    ...
    public setInvisibility(active: boolean) {
    this.invisible = active;
    this.styleData.opacity = active ? 0 : 1; // instantly invisible or visible
    }
...
// Apply gravity toward (0, 0) if the arena has the enableGravity flag

const arena = this.game.arena;
const flags = arena?.physicsData?.values?.flags ?? 0;

if ((flags & PhysicsFlags.enableGravity) !== 0) {
    console.log("Gravity enabled, applying force");
    const dx = -this.position.x;
    const dy = -this.position.y;
    const dist = Math.sqrt(dx * dx + dy * dy) || 1;

    const gravityStrength = 10;
    const pullX = (dx / dist) * gravityStrength;
    const pullY = (dy / dist) * gravityStrength;

    this.velocity.applyGravityForce(pullX, pullY);
    console.log(`"Gravity applied:", pullX.toFixed(2), pullY.toFixed(2)`);
}
