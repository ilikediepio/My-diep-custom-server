...
export const MultiAutoTurretDefinition: BarrelDefinition = {
    angle: 0,
    offset: 0,
    size: 65,
    width: 75 * 0.7,
...
export default class MultiAutoTurret extends MultiAutoBase {
    ...
    public constructor(owner: BarrelBase, turretDefinition: BarrelDefinition = MultiAutoTurretDefinition, baseSize: number = 20) {
        super(owner);
    ...
    public static createMultiAutoTurrets(owner: BarrelBase, count: number): MultiAutoTurret[] {
    const turrets: MultiAutoTurret[] = [];
    const radius = owner.sizeFactor * 200; // Adjust how far turrets appear from tank

    for (let i = 0; i < count; i++) {
        const angle = (i / count) * 2 * Math.PI;

        const turret = new MultiAutoTurret(owner);
