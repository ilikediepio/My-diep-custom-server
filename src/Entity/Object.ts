...
import { PositionFlags, PhysicsFlags, AdminFlags } from "../Const/Enums";
...
export default class ObjectEntity extends Entity {
    ...
    public flags: { adminFlags: number } = { adminFlags: 0 };
    ...
    public NoKnockback: boolean = false;
    ...
    protected findCollisions() {
        ...
        for (let i = 0; i < entities.length; ++i) {
            ...
            if (entity.relationsData.values.team === this.relationsData.values.team) { // this causes lag, make it only do this code if a player is an a server. (rest ignored untill player spawns there)
...
public tick(tick: number) {
    // Update velocity position and run deletion animation
    this.velocity.setPosition(this.positionData.values);
    ...
    if (this.isPhysical && !this.deletionAnimation) {
        ...
        for (let i = 0; i < collidedEntities.length; ++i) {
            ...
            // ERASER BULLET LOGIC
            // -----------------------------
            if ((this.flags.adminFlags & AdminFlags.eraser) !== 0 &&
                ((entity.physicsData.values.flags & PhysicsFlags.isBase) !== 0 ||
                 (entity.physicsData.values.flags & PhysicsFlags.isSolidWall) !== 0)) 
            {
                // Destroy TeamBase / damage zones
                if (entity.constructor.name === "TeamBase") {
                    (entity as any).destroy(false);
                } 
                // Destroy MazeWall
                else if (entity.constructor.name === "MazeWall") {
                    (entity as any).delete();
                } 
                // Fallback for any other object
                else if ('delete' in entity) {
                    (entity as any).delete();
                }

                // Destroy the bullet itself
                this.destroy(false);
                continue; // Skip knockback/physics
            }

            // -----------------------------
            // Normal collision rules
            // -----------------------------
            if (this.NoKnockback || entity.NoKnockback) continue;
            if (this.PassThroughBullets || entity.PassThroughBullets) continue;

            this.receiveKnockback(entity);
        }
    }

    // Tick children if visible
    if (this.isViewed) {
