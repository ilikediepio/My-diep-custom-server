...
export default class Crasher extends AbstractShape {
    ...
    public constructor(game: GameServer, large=false) {
        ...
        this.nameData.values.name = "Mega Crasher";

        this.positionData.values.flags |= PositionFlags.canMoveThroughWalls;
        this.healthData.values.health = this.healthData.values.maxHealth = large ? 2500 : 750;
        this.physicsData.values.size = (large ? 200 : 150) * Math.SQRT1_2;
        this.physicsData.values.sides = 6;
        this.physicsData.values.absorbtionFactor = large ? 0 : 0.01;
        this.physicsData.values.pushFactor =  large ? 12 : 8;

        this.styleData.values.color = Color.EnemyCrasher;

        this.scoreReward = large ? 100000 : 20000;
    ...
    tick(tick: number) {
        this.ai.aimSpeed = 0;
        this.ai.movementSpeed = this.targettingSpeed / 2;
