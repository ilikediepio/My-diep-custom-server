...
import { StyleFlags, AdminFlags, ClientBound } from "../Const/Enums";
...
export default class LivingEntity extends ObjectEntity {
    ...
    /** The base mass of obkects for gravity sim */
    public mass: number = 1;
    // disables collision
    public PassThroughBullets: boolean = false;
...
protected static applyDamage(entity1: LivingEntity, entity2: LivingEntity) { //start
    // Skip if either entity has pass-through bullets enabled
    if (entity1.PassThroughBullets || entity2.PassThroughBullets) return;
    if (entity2.healthData.values.health <= 0 || entity1.healthData.values.health <= 0) return;
    if (entity2.damagedEntities.includes(entity1) || entity1.damagedEntities.includes(entity2)) return;
    if (entity2.damageReduction === 0 && entity1.damageReduction === 0) return;
    if (entity2.damagePerTick === 0 && entity2.physicsData.values.pushFactor === 0 || entity1.damagePerTick === 0 && entity1.physicsData.values.pushFactor === 0) return;

    const game = entity2.game;

    // Damage forces swapped
    let dF1 = entity2.damagePerTick * entity1.damageReduction;
    let dF2 = entity1.damagePerTick * entity2.damageReduction;

    const is1Instakill = (entity1.flags.adminFlags & AdminFlags.instakill) !== 0;
    const is2Instakill = (entity2.flags.adminFlags & AdminFlags.instakill) !== 0;

    // Instakill logic unchanged
    if (is1Instakill && entity2.healthData.health > 0) {
        entity2.healthData.health = 0;
        return;
    }
    if (is2Instakill && entity1.healthData.health > 0) {
        entity1.healthData.health = 0;
        return;
    }

    // TankBody check swapped
    if (entity2 instanceof TankBody && entity1 instanceof TankBody) {
        dF1 *= 1.5;
        dF2 *= 1.5;
    }
    ...
    const ratio = Math.max(
        1 - entity2.healthData.values.health / dF2,
        1 - entity1.healthData.values.health / dF1
    );
    ...
    // Animation and damage application swapped

    // Plays the animation damage for entity1 (previously entity2)
    ...
    if (dF1 !== 0) {
        if (entity1.lastDamageTick !== game.tick && entity1 instanceof TankBody && entity1.definition.flags.invisibility && entity1.styleData.values.opacity < visibilityRateDamage) {
            entity1.styleData.opacity += visibilityRateDamage;
        }
        entity1.lastDamageTick = game.tick;
        entity1.healthData.health -= dF1;
    }

    // Plays the animation damage for entity2 (previously entity1)
    ...
    if (dF2 !== 0) {
        if (entity2.lastDamageTick !== game.tick && entity2 instanceof TankBody && entity2.definition.flags.invisibility && entity2.styleData.values.opacity < visibilityRateDamage) {
            entity2.styleData.opacity += visibilityRateDamage;
        }
        entity2.lastDamageTick = game.tick;
        entity2.healthData.health -= dF2;
    }
...
if (this.healthData.values.health <= 0) {
    ...
    // Siege Guardian defeat handling
    if (this.constructor.name === "SeigeGuardian") {

        const arena = this.game.arena as any;

        if (arena.state === 0) { // OPEN
    ...
    public tick(tick: number) {
        ...
        for (let i = 0; i < collidedEntities.length; ++i) {
            ...
            if (this.PassThroughBullets) continue;
