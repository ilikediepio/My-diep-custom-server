...
export const bossSpawningInterval = 5 * 60 * tps;
...
/** Get default access level depending on arena flags */
export function getDefaultAccessLevelForArena(game: GameServer | undefined): AccessLevel {
    const arenaFlags = game?.arena?.arenaData?.values?.flags;
    
    if (arenaFlags == null) {
        // No arena info? Use your normal default
        return defaultAccessLevel;
    }

    // If cantUseCheats flag is set, restrict to PublicAccess
    if (arenaFlags & ArenaFlags.cantUseCheats) {
        return AccessLevel.PublicAccess;
    }
