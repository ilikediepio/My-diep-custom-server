...
export default class Velocity extends Vector {
    ...
    // NEW: gravity force accumulator
    private gravity = new Vector(0, 0);

    public updateVelocity() {
        // Update based on position delta
        ...
        // Optional: damp gravity slightly for smoother feel
        this.gravity.x *= 0.98;
        this.gravity.y *= 0.98;
    }
    ...
    // NEW: public method to add gravity
    public applyGravityForce(x: number, y: number) {
        this.gravity.x += x;
        this.gravity.y += y;
    }
}