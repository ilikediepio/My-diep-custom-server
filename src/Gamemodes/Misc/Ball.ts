...
class CustomShapeManager extends SandboxShapeManager {
    protected spawnShape(): AbstractShape {
        ...
        const hexa = new Hexagon(this.game, Math.random() < 0.25, Math.random() < 0.1);
        ...
        hexa.positionData.values.x = Math.sign(x) * (Math.abs(x) - 200);
        hexa.positionData.values.y = Math.sign(y) * (Math.abs(y) - 200);
        hexa.relationsData.values.owner = penta.relationsData.values.team = this.arena;
